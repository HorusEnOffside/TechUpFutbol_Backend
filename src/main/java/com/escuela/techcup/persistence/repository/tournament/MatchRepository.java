...
public interface MatchRepository extends JpaRepository<MatchEntity, String> {
    ...
    @Query("SELECT m FROM MatchEntity m WHERE m.id = :matchId " + "AND (m.teamA.id = :teamId OR m.teamB.id = :teamId)")
    Optional<MatchEntity> findByIdAndTeam(@Param("matchId") String matchId, @Param("teamId") String teamId);
